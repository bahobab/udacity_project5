...
from forms import NewPublisher, EditPublisher, NewAuthor, EditAuthor, NewCategory, EditCategory, NewReview, BookByIsbn
import urllib2
import os
from werkzeug import secure_filename

#---------------- support for image upload and processing -------------
UPLOAD_FOLDER = '/vagrant/catalog/static/img/'
...
GOOGLE_BOOK_API_URL = 'https://www.googleapis.com/books/v1/volumes?q=isbn:'

app = Flask(__name__)
# create csrf object
csrf = SeaSurf(app)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
#limit uploads size to 5 MB
...
## ----- Test WTForms ---------
#@app.route('/books/testnewbook/', methods = ['Post', 'Get'])
#def testNewBook():
#    form = NewBook()
...
#
#    cat_obj = session.query(Category).first()
#
#    form.category.choices = [(category.id, category.name) for category in categories]
#    return render_template('WTFNewbook.html',
#                           form = form,
#                           class_object = cat_obj,
#                           class_name = 'Category')
#
##------- end WTForm Test -----
...
def gdisconnect():
    ...
    if credentials is None:
        ...
        return response
#    access_token = credentials.access_token
#    convert credentials string into a dictionary
...
# ----------------- begin controller-view functions------------------

def login_required(f):
    """ Login required helper function """
    @wraps(f)
...
#        return redirect('/login')
    ...
    if request.method == 'POST':
        if (check_csrf(login_session.pop('_csrf_token', None), request.form.get('_csrf_token'))):
            if request.form['name']:
                try:
                    # first check if already exist to avoid duplicate
    ...
    return render_template('newcategorywtf.html', form = form)
...
#        return redirect('/login')
    ...
    if login_session['user_id'] == categoryToEdit.user_id:
        if request.method == 'POST':
            ...
            return render_template('editcategorywtf.html',
...
#
@app.route('/books/isbn/<class_name>/<int:class_id>/')
@login_required
def newBookByIsbn(class_name, class_id):
    """ open form to get book isbn """
    form = BookByIsbn()
    return render_template('pullfromgoogleapiwtf.html',
...
def newBook(class_name, class_id):
    ...
    Only POST method accesses here newBookByIsbn
    """
#    if 'username' not in login_session:
#        return redirect('/login')
    if (check_csrf(login_session.pop('_csrf_token', None), request.form.get('_csrf_token'))):
        if request.form.has_key('isbn_google_api'):
            # the POST is coming from the ISBN form
    #        try:
            categories = session.query(Category).all()
            authors = session.query(Author).all()
            publishers = session.query(Publisher).all()
    #        except:
    #        flash('FAILED Step 1: New Book Could Not Be Added')
    ...
    #            return redirect(url_for('booksByPublisher', publisher_id = publisher_id))
        ...
        # Get an obbject by the class id
            ...
            bookInfo = dict()
            try:
                isbn = bleach.clean(request.form['isbn_google_api'])
                if isbn:
                    # first, clean it up
                    isbn = isbn.replace(' ', '')
                    isbn = isbn.replace('-', '')
                    isbn = isbn.strip()
                    url = urllib2.urlopen(GOOGLE_BOOK_API_URL+isbn)
                    bookJSON = url.read()
#                    print '<<<<<<<<<<<<<<<<', bookJSON
                    jsonToDict = json.loads(bookJSON)
                    if jsonToDict['totalItems'] == 0:
                        raise ValueError('ISBN Not Found')
                    bookInfo = jsonToDict['items'][0]
                    bookInfo = bookInfo['volumeInfo']
#                    print '<<<<<<<<<<<<<<<<', bookInfo
                    flash('Pre-populate Book Info With Google API Data. Complete Empty Fields')
                    return render_template('newbookwithapi.html', class_object = class_object,
                                               class_name = class_name,
                ...
                # No isbn provided or error processing it
                flash('No Google API Data Available... Provide Data Yourself')
        ...
        if imgFile and allowed_file(imgFile.filename):
            ...
            imgFile.save(app.config['UPLOAD_FOLDER'] + os.path.relpath(filename))
            imageURL = '/static/img/' + os.path.relpath(filename)
        else:
            imageURL = IMG_COVER_SRC + DEFAULT_COVER_IMG
        try:
            # now we can try add the new book
            newBook = Book(
            ...
            flash('FAILED: New Book Could Not Be Added')
            if class_name == 'Category':
                return redirect(url_for('booksByCategory', category_id = category_id))
            elif class_name == 'Author':
                return redirect(url_for('booksByAuthor', author_id = author_id))
        ...
        flash('You Do Not Have Permissions To Complete This Operation')
...
#        return redirect('/login')
    ...
    if bookToEdit.user_id == login_session['user_id']:
        if request.method == 'POST':
            if (check_csrf(login_session.pop('_csrf_token', None),
                           request.form.get('_csrf_token'))):
                # process uploaded file, check if file allowed and sanitize file name
                imgFile = request.files['imageURL']
                if imgFile and allowed_file(imgFile.filename):
                    filename = secure_filename(imgFile.filename)
                    filename = os.path.abspath(filename)
                    imgFile.save(app.config['UPLOAD_FOLDER'] + os.path.relpath(filename))
                    bookToEdit.imageURL = UPLOADED_FOLDER + os.path.relpath(filename)
                try:
                    ...
                    flash('Book Could not Be Edited')
                    if class_name == 'Category':
                        return redirect(url_for('booksByCategory',
                                                category_id = bookToEdit.category_id))
                    elif class_name == 'Author':
...
#                    deleteBookReviews(bookToDelete)
                    ...
                    flash('FAILED: Book Could Not Be Deleted...')
                    if class_name == 'Category':
                        return redirect(url_for('booksByCategory', category_id = category_id))
                    elif class_name == 'Author':
                        return redirect(url_for('booksByAuthor', author_id = author_id))
...
def reviewBook(book_id, class_name):
            ...
            if request.form['review']:
                try:
                    newReview = Review(
                        ...
                        user_id = login_session['user_id'],
                        reviewer_name = login_session['username'],
    ...
    return render_template('reviewbook.html',
                           ...
                           reviews = reviews,
                           class_name = class_name,
                           user_id = login_session['user_id'], form = form)
...
#        return redirect('/login')
    ...
    if login_session['user_id'] == reviewToDelete.user_id:
        try:
        ...
        if (check_csrf(login_session.pop('_csrf_token', None), request.form.get('_csrf_token'))):
            if request.form['name']:
                ...
                try:
                    ...
                    flash('Failed: New Publisher Could Not Be Added...')
            else:
                flash('Failed: Please Fill Out Publisher Name Field')
            return redirect(url_for('publishers'))
    return render_template('newPublisherwtf.html', form = form)
        ...
        if request.method == 'POST':
            if (check_csrf(login_session.pop('_csrf_token', None), request.form.get('_csrf_token'))):
                ...
                try:
                    ...
                    flash('FAILED: Publisher Could Not Be Edited...')
                return redirect(url_for('publishers'))
        else:
            # GET request
            return render_template('editpublisherwtf.html',
            ...
            if (check_csrf(login_session.pop('_csrf_token', None),
                ...
                if isFreeToDelete(publisherToDelete, "Publisher"):
                    ...
                    try:
                        ...
                        flash('FAILED: Publisher Could Not Be Deleted...' %publisherToDelete.name)
                else:
        ...
        if (check_csrf(login_session.pop('_csrf_token', None), request.form.get('_csrf_token'))):
            if request.form['name']:
                ...
                try:
                    ...
                    flash('FAILED: New Author Could Not Be Created...')
            else:
                flash('Failed: Please Fill Out Author Name Field')
            return redirect(url_for('authors'))
    return render_template('newAuthorwtf.html', form = form)
...
# --------- helper function to check for valid CSRF  ---------
...
# --------- helper function for deleting a class ---------
def isFreeToDelete(class_object, class_name):
    """ Check if there are no books by author or publisher
    By design authors, categories and publisher can be deleted
    ONLY IF NO LONGER in use by  books.
    Input: author, category or publisher object, class name (Author, Category, Publisher)
    Return: True: no more books by publisher or author or category. Or False otherwise
    """
    if class_name == 'Author':
        try:
...
# ---------------- Helper function to check/create a class item if not already exist
...
#        print '>>not found..created...', category.name
        return category.id
    if itemClass == 'Author':
        newAuthor = Author(name = itemName,
                           user_id = login_session['user_id'],
                          imageURL = IMG_AUTHOR_SRC + DEFAULT_AUTHOR_IMG)
        session.add(newAuthor)
        session.commit()
...
def allowed_file(filename):
    """ lets make sure we allows the file being uploaded """
    return '.' in filename and filename.rsplit('.', 1)[1] in ALLOWED_EXTENSIONS


# ----------------- Helper function to delete reviews belonging to a book -----
